    ...
    for group in paginated_groups:
        ...
        with st.container():
            ...
            with st.expander("View Versions"):
                st.write("### Versions")
                st.dataframe(group[[
                    "community", "location", "budget_item", "contractor",
                    "classification", "proposed_budget", "change_order",
                    "revised_budget", "status", "date_executed"
                ]])
...
def render_land_development_ui():
    st.title("🏗️ Land Development")
    view = st.sidebar.radio("Select View", ["Budget Summary", "Details", "Preview All"])
...
def render_add_entry():
    ...
    with st.form("add_entry_form"):
        ...
        classification = st.selectbox("Classification", ["Original", "Revised"])
        ...
        date_executed = st.date_input("Date Executed")
...
def render_edit_form(row):
    with st.form(f"edit_form_{row['id']}"):
        ...
        classification = st.selectbox("Classification", ["Original", "Revised"], index=["Original", "Revised"].index(row["classification"]))
        ...
        date_executed = st.date_input("Date Executed", value=pd.to_datetime(row["date_executed"]))
...
# --- Update Details View to Include Edit ---
def render_details_view():
    st.header("📋 Development Details")
